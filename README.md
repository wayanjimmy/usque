...
There is a handy, though not very feature-rich, `register` subcommand that creates a fresh Warp account ready to use. It also takes care of device registration and MASQUE device enrollment. Call this once and it will create a working configuration for future use in modules.
...
💡 This command refreshes your config with data downloaded from the Cloudflare servers, so make sure you have backups.
💡 When using ZeroTrust this command can update the config with newly assigned IPv4 and IPv6 addresses. This is useful, because IPv6 doesn't seem to work there if the IPv6 in config isn't up to date. Personal WARP doesn't seem to be affected by this.
...
To bring up a native tunnel, you can run:

```shell
$ sudo ./usque nativetun
```

You should see a `tun0` (or tun1, tun2, etc.) interface appear. If you didn't disable IPv4 and IPv6 inside the tunnel using cli flags, you should also see the IPv4 and IPv6 address pre-assigned to this interface. This alone should be enough for applications that can route traffic through a specific network interface to function. For example `ping`:
...
Should work. However **the tool doesn't set any routes**. If you need that, you have to do that manually. For example, to route all traffic to the tunnel, you first need to make sure that the address used for establishing the tunnel is routed to your regular network interface. For that, open the `config.json` and check the endpoint address. If you plan to connect to the Cloudflare endpoint using IPv4, you will most likely see this:
...
💡 Always be careful with default routes, especially if you are running this on a headless machine. You can very easily close yourself out of your current session. I suggest [network namespaces](https://man7.org/linux/man-pages/man7/network_namespaces.7.html) as a safer playground for experiments or a spare VM with physical access/serial.
...
Which means that your device's internal IPv4 address is `100.96.0.3`. You are running a webserver on the host machine on port `8080`. Then there is another device with internal IPv4 address `100.96.0.2` that also exposes a webserver on port `8081`. You would like to forward the host's port `8080` to the WARP network and the WARP network's port `8081` to the host. You can do that like this:
...
💡 Any number of ports are supported. You can chain more together by specifying the flag and the corresponding argument one after another.
...
In my view ZeroTrust is Cloudflare's enterprise version of WARP. Explaining what it is would be out of scope for this README.

While the tool won't be able to log you in to ZeroTrust, as it requires SSO, practice shows that you can get connection working if you really want to. For that you need to put together a config file manually. I suggest you use the `register` command to get a personal WARP config, keep all fields untouched except for `access_token` and `id`.  As for how to get these, get creative.  For example both of these can be carved out from `/var/lib/cloudflare-warp/reg.json` if using the official WARP client on Linux. Or existing device IDs are listed in the ZeroTrust dashboard. Once these are in place, you can use the `enroll` command to refresh the config with the new data. You will see that the `license` field is empty. This is normal. ZeroTrust doesn't use licenses (to my knowledge).
...
The project is still in early stages *(I am happy I even got it working)* and performance wasn't a priority. In fact I am not even too familiar with Go. The official client *(at least on Linux and Android)* is implemented in Rust with the awesome [quiche](https://github.com/cloudflare/quiche) project. While this tool is written in Go and uses the awesome [quic-go](https://github.com/quic-go/quic-go) library which is very well maintained and has a broad support for the QUIC protocol. However it only supports `reno` congestion control and it isn't the most performant implementation out there especially for high latency environments.

Many other features such as [happy eyeballs](https://en.wikipedia.org/wiki/Happy_Eyeballs) is not implemented.

So yes, the performance might not be the best. However I was able to squeeze out 833.60 Mbps download and 772.88 Mbps upload on a 1 Gbps connection with Warp+ upon the first try using the SOCKS5 proxy mode with Firefox and [speedtest.net](https://www.speedtest.net/). The test was conducted on an `AMD Ryzen 7 5700U` with `16 GB` of RAM on `Arch Linux`. Which is good enough for me. I am sure there is room for improvement. But keep in mind that this is all userspace; SOCKS mode even emulates its own network stack. CPU usage was around 26%.
...
`quic-go` will nicely warn you if this is set too small on your machine, but the default UDP buffer size on Linux is quite small. You can increase it by running:
...
By default all modes except for the native tunnel mode will use [Quad9](https://quad9.net/) inside the tunnel to resolve DNS traffic. While this seems to be an odd choice for a Cloudflare client, I prefer them over `1.1.1.1` because of their privacy claims. I believe it's a decent default. However `1.1.1.1` usually has better performance. You are free to change the DNS server used by the tool by specifying the `-d` flag
...
- **remote end disconnects**: If you are inactive for a while, the remote end might disconnect you with a `H3_NO_ERROR` error. Similar behavior was observed earlier on their well studied `wireguard` implementation where too long open connections with not much network activity were disconnected. While the official client seems to be able to keep the connection alive, this tool doesn't. I know the reason, but unsure which path to take to fix it yet. Regular browsing using SOCKS for example usually produces enough traffic that the tunnel remains stable for a long time. **As a workaround please do not consider generating artificial traffic as this is bad for the environment and we don't want to DoS Cloudflare either. Remember, this service is also offered for free, and with abuse, you will most likely get this tool sanctioned.** Wait for a fix instead.
...
There is hardly a way to distinguish MASQUE traffic from other HTTP/3 traffic. However QUIC mandates TLS v1.3 so we send a ClientHello with `client-masque.cloudflareclient.com` as SNI. Some firewalls might block this. You can change the SNI by specifying `-s` flag to any domain based on my experience and the connection will still work. Please note that this is definitely not Cloudflare's intended use case *(just a nice side effect)*. And before doing any circumvention attempts, you should make sure you are not breaking any laws. Personally I only see this as a clear benefit for masking the fact that we are connecting to Warp from MiTMers.

## Should I replace WireGuard with this?

That depends on your needs. 😊 WireGuard is a great protocol and its cryptographic simple cryptographic set plus the ability to have kernel mode support are great things. If it works for you, I don't believe you should switch.
...
- **reports that WARP+ doesn't work anymore over WireGuard arose lately**: I have seen reports that WARP+ doesn't work anymore over WireGuard. While I cannot confirm this, I have seen reports that it is the case ([see](https://github.com/ViRb3/wgcf/issues/459#issuecomment-2645319671)). It seems to work for me with this tool, so I assume there is that...
...
This document would be large and too horrific for the average reader if I included all the details about the protocol and the research I did. If you are one of the few who are interested in some details, please refer to the [RESEARCH.md](RESEARCH.md) file. That one summarizes the research I did and the protocol details I found in a blog post like format. In the future I plan to write a clear and concise protocol document too that will be linked here.

## Why was this built?

Mostly because I wanted to experiment with cool new technology, and MASQUE caught my attention. I was shocked to see that there are not many open-source implementations out there, in fact I haven't really seen any practical uses for `connect-ip` even yet. I wanted to change that and *"advertise"* a bit.
...
I figured this while sitting on the train while browsing the interwebz casually with routing the entire traffic. Once in a while it would crash and disconnect the entire VPN profile leaving my traffic exposed to the local WiFi. That was quite unpleasant and the final straw that made me start this project.
...
Because Cloudflare's implementation isn't exactly RFC 9484 compliant and it's not going to work without directly monkey patching the library. Find my ugly, but hopefully working [here](https://github.com/Diniboy1123/connect-ip-go).

## Why the name?

Obviously I didn't plan to name it `warp`, `cloudflare` or anything similar to reduce conflicts or cause users to knock on the wrong doors for support because of my faulty implementation attempts.

Since the underlying technology is MASQUE, I wanted to come up with something short, memorable and unique, but still somehow related to the underlying technology. I had a fun 4 years of Latin in school, so I figured the perfect name would be `usque`. If I can trust my rusty memory, it means `all the way`. I thought it was a good fit, because it is a tool that tunnels all the way to the Cloudflare network. We also go a long way until this becomes stable. You get the idea... 😊  It is also (hopefully) unique enough to not conflict with other projects.
